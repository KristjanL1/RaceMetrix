...
def save_lap_time(track_name, lap_time, driver_name):
    #print("laptime_handler.py: savelaptime outside of try.")
    try:
        print("laptime_handler.py: Saving laptime:", lap_time, "at",track_name, "by", driver_name)
        # Call the function to save lap time to the database
        save_lap_time_to_db(track_name, lap_time, driver_name )
...
def display_lap_times():
    ...
    try:
        ...
        if lap_times:
            ...
            formatted_lap_times = "\n".join([f"Track: {track_name}, Lap Time: {lap_time} By {driver_name}" for track_name, lap_time, driver_name in lap_times])
